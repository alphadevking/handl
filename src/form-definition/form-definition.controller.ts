...
export class FormDefinitionController {
   ...
   */
  @Get(':name')
  findOne(@Req() req: Request, @Param('name') name: string): Promise<FormDefinition> {
    const userId = req.user!.id; // Assert that req.user is not undefined
    return this.formDefinitionService.findOne(userId, name);
  }

  /**
   * Updates an existing form definition by its name (ID) for the authenticated user.
   * @param req The request object containing the authenticated user.
  ...
  update(
    ...
    const userId = req.user!.id; // Assert that req.user is not undefined
    return this.formDefinitionService.update(userId, name, updateFormDefinitionDto);
  }

  /**
   * Deletes a form definition by its name (ID) for the authenticated user.
   * @param req The request object containing the authenticated user.
   * @param name The name (ID) of the form definition to delete.
   */
  @Delete(':name')
  @HttpCode(HttpStatus.NO_CONTENT)
  remove(@Req() req: Request, @Param('name') name: string): Promise<void> {
    const userId = req.user!.id; // Assert that req.user is not undefined
    return this.formDefinitionService.remove(userId, name);
  }
}
